...
delete from owner; -- refreshing the table -- Be careful with this form of delete

-- multiple rows may be iadded with one INSERT statement by repeating the VALUES clasue
...
--
--
-- Be sure any foreign key values have matching data in the primary key
--
-- name 		pet_type 				owner
-- Jenna		German Shepard Dog		Frank
-- Princess 	Tea Cup Dog				Jay
-- Chloe		Siamese	Cat				Cameron
...
-- use a sub-query/sub0select to have the database manager find the value for us
-- (A sub-query is just a select inside of another SELECT, INSERT, UDPATE, or DELETE
--
-- To get the pet_type_id for a German Shepard Dog
--
--select pet_type_id from pet_types where name = 'German Shepard' and species = Dog;
--
-- to get owner_id for Frank the select would be:
--
